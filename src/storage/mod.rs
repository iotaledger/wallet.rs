...
mod tests {
    ...
    async fn set_adapter() {
        ...
        impl StorageAdapter for MyAdapter {
            async fn get(&mut self, _key: &AccountIdentifier) -> crate::Result<String> {
                Ok("MY_ADAPTER_GET_RESPONSE".to_string())
            }
            async fn get_all(&mut self) -> crate::Result<Vec<String>> {
                Ok(vec![])
            }
            async fn set(&mut self, _key: &AccountIdentifier, _account: String) -> crate::Result<()> {
                Ok(())
            }
            async fn remove(&mut self, _key: &AccountIdentifier) -> crate::Result<()> {
                Ok(())
            }
        }
