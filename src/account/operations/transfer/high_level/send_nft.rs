...
impl AccountHandle {
    ...
    /// [`StorageDepositReturnUnlockCondition`](iota_client::bee_message::output::unlock_condition::StorageDepositReturnUnlockCondition)
    /// and [`ExpirationUnlockCondition`](iota_client::bee_message::output::unlock_condition::ExpirationUnlockCondition),
    /// so the storage deposit gets back to the sender and also that the sender gets access to the output again after a
    /// defined time (default 1 day), Calls [AccountHandle.send()](crate::account::handle::AccountHandle.send)
    /// internally, the options can define the RemainderValueStrategy.
    /// Custom inputs will be replaced with the required nft inputs.
