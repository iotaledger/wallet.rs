...
impl AccountHandle {
    ...
    pub(crate) async fn get_outputs(&self, output_ids: Vec<OutputId>) -> crate::Result<(Vec<OutputResponse>, u64)> {
        ...
        // drop account early so it's not blocking it while sending the requests
        drop(account);
