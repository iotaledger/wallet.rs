...
use iota_client::{block::output::OutputId, node_api::indexer::query_parameters::QueryParameter, Client};

use crate::account::handle::AccountHandle;

/// Returns output ids of basic outputs that have only the address unlock condition
pub(crate) async fn get_basic_output_ids_with_address_unlock_condition_alone(
    client: &Client,
    bech32_address: String,
) -> iota_client::Result<Vec<OutputId>> {
    // Only request outputs with basic unlock conditions and only `AddressUnlockCondition`
    client
...
impl AccountHandle {
    /// Returns output ids of basic outputs that have the address in any unlock condition
