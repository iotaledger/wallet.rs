...
* Create instance of `iota_wallet.Transfer()` class with the following mandatory arguments: `amount`, `address`, `bech32_hrp` and `remainder_value_strategy`
* `bech32_hrp` indicates which network should be used to send tokens thru, and can be gotten from `account.bech32_hrp()` function
