...
impl AccountManager {
    ...
    fn create_account(&self, client_options: ClientOptions) -> Result<AccountInitialiser> {
        Ok(AccountInitialiser {
            account_initialiser: Some(
                self.account_manager
                    .as_ref()
                    .expect("account_manager got destroyed")
                    .create_account(client_options.into())?,
            ),
    ...
    fn sync_accounts(&self) -> Result<AccountsSynchronizer> {
        let accounts_synchronizer = self
            .account_manager
            .as_ref()
            .expect("account_manager got destroyed")
            .sync_accounts()?;
    ...
    fn backup(&self, destination: &str, password: &str) -> Result<String> {
        Ok(crate::block_on(async {
            self.account_manager
                .as_ref()
                .expect("account_manager got destroyed")
                .backup(destination, password.to_string())
                .await
        })?
        .into_os_string()
        .into_string()
        .unwrap_or_else(|os_string| panic!("invalid backup result {os_string:?} with destination: {destination:?}",)))
    }
    ...
    fn get_account(&self, account_id: &str) -> Result<AccountHandle> {
        let account_handle = crate::block_on(async {
            self.account_manager
                .as_ref()
                .expect("account_manager got destroyed")
    ...
    fn get_accounts(&self) -> Result<Vec<AccountHandle>> {
        let account_handles = crate::block_on(async {
            self.account_manager
                .as_ref()
                .expect("account_manager got destroyed")
                .get_accounts()
    ...
    /// Removes the stronghold snapshot from memory and clears the password.
    fn lock_stronghold(&self) -> Result<()> {
        crate::block_on(async {
            Ok(iota_wallet::lock_stronghold(
                &self
                    .account_manager
                    .as_ref()
                    .expect("account_manager got destroyed")
            ...
            .await
            .map_err(iota_wallet::Error::StrongholdError)?)
        })
    }

    /// Locks stronghold and destroys the account manager.
    #[allow(unused_must_use)]
    fn destroy(&mut self) -> Result<()> {
        crate::block_on(async {
            let manager = &self.account_manager.as_ref().expect("account_manager got destroyed");

            iota_wallet::lock_stronghold(&manager.stronghold_snapshot_path().await?, true)
                .await
                .map_err(iota_wallet::Error::StrongholdError)?;

            iota_wallet::remove_storage(manager.storage_path())
                .await
                .expect("couldn't remove storage instance");
