...
import b_list_blocks from  '!!raw-loader!./../../../../bindings/python/examples/4b_list_blocks.py';
...
* When you initialize the new database, a Stronghold mnemonic (seed) is automatically generated and stored by default ( `manager.storeMnemonic(secret_manager.Stronghold)` ).
...
* Other data used by library that is stored under the _db_ sub-directory.  The includes account information, generated addresses, fetched blocks, etc.  This data is used to speed up some operations, such as account creation, address generation, etc.
...
1. Create instance of `iota_wallet.Transaction()` class with the following mandatory arguments: _amount_, _address_ and _remainder_value_strategy_ .
The _remainder_value_strategy_ argument can be either:
   - `ReuseAddress`
   - `ChangeAddress`
2. Once you have created an instance of `iota_wallet.Transaction()` , you can send the tokens using the `transfer()` function of the _Account_ instance.
            ...
            'essence': {
                'regular': {
                    'inputs': [{
                        ...
                        'metadata': {
                            'transaction_id': '692d6660084dd3b6341ef4f761bc8b8bb27ac35bb0b352bfb030f2c80753815b',
                            'block_id': 'c6284e0cc2a6383474782d4e6b6cfaf16c1831c8875cca262982782758a248c0',
                            'index': 0,
                            'amount': 10000000,
                            'is_spent': False,
                            'address': {
...
```
This is a _wallet block_ that fully describes the given transaction.

To understand all aspects of blocks, you will need to get familiar with concept of _UTXO_ . You can find detailed information in the [UTXO section in the Developer Guide to Chrysalis](https://chrysalis.docs.iota.org/guides/dev_guide#unspent-transaction-output-utxo).

You can double-check the block using [Tangle Explorer](https://explorer.iota.org/) using its _node_response['id']_.  Please make sure you select the right network.

If you have used the _ChangeAddress remainder_value_strategy_, the block will transfer tokens to the target address as well as new _internal_ address within the given account (`internal=True`).

You can find detailed information about blocks and payloads in the [Developer Guide to Chrysalis](https://chrysalis.docs.iota.org/guides/dev_guide#blocks-payloads-and-transactions).

### Reattachments

If you need to reattach a block, you should use the [`iota_wallet.promote(account_id, block_id)`](api_reference.md#promoteaccount_id-block_id-walletblockwalletblock) or [`iota_wallet.reattach(account_id, block_id)`](api_reference.md#reattachblock_id-walletblockwalletblock) methods, sending your _account_id_ and _block_id_ as arguments.

### List of Blocks (transactions)

You can query for a list of all particular blocks (transactions) related to the given account using [ `account.list_blocks()` ](api_reference.md#list_blockscount-from-block_type-optional-listwalletblockwalletblock) method, and the related [ `account.block_count()` ](api_reference.md#block_countblock_type-optional-int) method.

You can use those methods to check whether a block is confirmed, broadcast, etc. You should always _Sync_ the account with the Tangle before checking confirmation status.

You can use the following example to _sync_ an _account_ , and list all the blocks related to the _account_ .
