...
All of the examples in this section are based on you having set your custom password in the _.env_ file:
...
* The seed should only be for the first time. You can open an already initialized database using your password.
...
* A single file called _wallet.stronghold_, which contains the _seed_. `Stronghold` will secure the seed and encrypt it at rest. The generated seed (mnemonic) serves as a cryptographic key, which is used to generate all accounts and related addresses.
...
You can find detailed information about seed generation in the [Developer Guide to Chrysalis](https://chrysalis.docs.iota.org/guides/dev_guide#seed).

### Accounts

The `wallet.rs` library uses a model of individual accounts to separate individual users or clients from each other. It is possible to generate multiple addresses for each account deterministically. You can find more information about account management in the [Developer Guide to Chrysalis](https://chrysalis.docs.iota.org/guides/exchange_guide#how-do-i-implement-it-to-my-exchange).
...
You can refer to the [Wallet NodeJs API Reference](api_reference.md) for more information about _clientOptions_ .
...
The network uses a [dust protection](https://wiki.iota.org/chrysalis-docs/faq#what-is-dust-protection-and-how-does-it-work) protocol to prevent malicious actors from spamming the network while also keeping track of the unspent amount ( _UTXO_ ).
