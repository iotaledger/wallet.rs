...
Therefore, pay attention to the work with transaction outputs, understand what kind of unlock conditions exist and test your integration properly.

Examples:
- If a transaction produces an output that can be unlocked using of an "expiration unlock condition", the sender could claim the money back if the recipient did not claim it in time.
- If the transaction produces an output that can be unlocked using "timelock unlocking condition", it might be the case that receiver can claim the funds only after a very long time.
