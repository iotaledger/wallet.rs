...
- if the transaction produces an output that can be unlocked using an "timelock unlocking condition", it might be the case that receiver can claim the funds never or only after a long time
