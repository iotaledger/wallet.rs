...
IOTA allows for complex tokenization schemes, but they can lead to monetary losses if used improperly or integrated incorrectly.
Transaction outputs may have multiple unlocking conditions which may require returning some or all of the amount, which could expire if not claimed in time, or which may not be unlockable for a very long time.
Therefore, pay attention when working with transactions, understand what kind of unlock conditions exist and test your integration properly.

Examples:
- if the transaction produces an output that can be unlocked using an "expiration unlock condition", the sender could claim the money back if the recipient did not claim it in time
- if the transaction produces an output that can be unlocked using an "timelock unlocking condition", it might be the case that the receiver never can claim the funds or only after a long time
